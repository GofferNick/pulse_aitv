...
cur.execute("select count(*) from clients where active = '1' and end_date > CURRENT_DATE  and tarif_plan_id in ('184','185','186','187','188','189','190','191','117', '120','168','169','170','171');")
...
cur.execute("select count(*) from operations_log, clients where ((EXTRACT(month FROM operations_log.change_time) = '12') and (EXTRACT(year FROM operations_log.change_time) = '2017')) and  clients.tarif_plan_id in('184', '185', '186', '187', '188', '189', '190', '191') and clients.active = '1' and operations_log.new_value = '1' and operations_log.column_name = 'active' and clients.id = operations_log.row_id and ((date_part('days', (current_timestamp - first_activation_date)) > 60) or clients.first_activation_date is null);")
came_back = cur.fetchone()[0]

cur.execute("select count(*) from operations_log, clients where ((EXTRACT(month FROM operations_log.change_time) = '12') and (EXTRACT(year FROM operations_log.change_time) = '2017')) and clients.id = operations_log.row_id  and clients.active = '0' and operations_log.new_value = '0' and operations_log.column_name = 'active';")
deactiv_full = cur.fetchone()[0]
cur.execute("select count(*) from clients where (EXTRACT(month FROM first_activation_date) = '12') and (EXTRACT(year FROM first_activation_date) = '2017');")
...
print("Вернувшихся в этом месяц: " + str(came_back))
